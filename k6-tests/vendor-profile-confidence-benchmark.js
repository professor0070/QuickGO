...
const vendorDashboardLatency = new Trend('vendor_dashboard_latency');
const vendorProfileLatency = new Trend('vendor_profile_latency');
const vendorDashboardFailures = new Rate('vendor_dashboard_failures');
const vendorProfileFailures = new Rate('vendor_profile_failures');
...
export const options = {
  ...
  thresholds: {
    ...
    vendor_dashboard_latency: ['p(95)<500', 'p(99)<750'],
    vendor_profile_latency: ['p(95)<500', 'p(99)<750'],
    // Failure rate limits
    vendor_dashboard_failures: ['rate==0'],
    vendor_profile_failures: ['rate==0'],
  },
};
...
export function setup() {
  ...
  const authOk = check(response, {
    ...
    '[setup_auth] user has VENDOR_OWNER or VENDOR_STAFF role': () => {
      if (!isJson || !jsonParsed || !jsonParsed.data || !jsonParsed.data.user) return false;
      const roles = jsonParsed.data.user.roles;
      if (typeof roles === 'string') return roles.indexOf('VENDOR_OWNER') !== -1 || roles.indexOf('VENDOR_STAFF') !== -1;
      if (Array.isArray(roles)) return roles.includes('VENDOR_OWNER') || roles.includes('VENDOR_STAFF');
      return false;
    },
  });
...
function runValidation(response, endpointName, isDashboard) {
  ...
  if (isDashboard) {
    checksObj[`[${endpointName}] contains dashboard keys`] = () => isJson && jsonParsed && jsonParsed.data && ('vendor_id' in jsonParsed.data) && ('shop_open' in jsonParsed.data);
  } else {
    checksObj[`[${endpointName}] contains profile keys`] = () => isJson && jsonParsed && jsonParsed.data && ('id' in jsonParsed.data) && ('shopName' in jsonParsed.data);
  }
...
export function runWarmup(data) {
  ...
  const dashRes = http.get(
    `${cleanBaseUrl}/api/v1/vendor/dashboard`,
  ...
  const profileRes = http.get(
    `${cleanBaseUrl}/api/v1/vendor/profile`,
    Object.assign({}, authParams, { tags: { name: 'warmup_profile' } })
  );
  runValidation(profileRes, 'warmup_profile', false);
...
export function runMeasured(data) {
  ...
  const dashRes = http.get(
    `${cleanBaseUrl}/api/v1/vendor/dashboard`,
    Object.assign({}, authParams, { tags: { name: 'vendor_dashboard' } })
  );
  const dashOk = runValidation(dashRes, 'vendor_dashboard', true);
  vendorDashboardLatency.add(dashRes.timings.duration);
  vendorDashboardFailures.add(dashOk ? 0 : 1);

  // GET /api/v1/vendor/profile
  const profileRes = http.get(
    `${cleanBaseUrl}/api/v1/vendor/profile`,
    Object.assign({}, authParams, { tags: { name: 'vendor_profile' } })
  );
  const profileOk = runValidation(profileRes, 'vendor_profile', false);
  vendorProfileLatency.add(profileRes.timings.duration);
  vendorProfileFailures.add(profileOk ? 0 : 1);
