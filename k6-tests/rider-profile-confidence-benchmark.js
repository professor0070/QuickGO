...
const riderDashboardLatency = new Trend('rider_dashboard_latency');
const riderProfileLatency = new Trend('rider_profile_latency');
const riderDashboardFailures = new Rate('rider_dashboard_failures');
const riderProfileFailures = new Rate('rider_profile_failures');
...
export const options = {
  ...
  thresholds: {
    ...
    rider_dashboard_latency: ['p(95)<500', 'p(99)<750'],
    rider_profile_latency: ['p(95)<500', 'p(99)<750'],
    // Failure rate limits
    rider_dashboard_failures: ['rate==0'],
    rider_profile_failures: ['rate==0'],
  },
};
...
export function setup() {
  ...
  const authOk = check(response, {
    ...
    '[setup_auth] user has RIDER role': () => {
      if (!isJson || !jsonParsed || !jsonParsed.data || !jsonParsed.data.user) return false;
      const roles = jsonParsed.data.user.roles;
      if (typeof roles === 'string') return roles.indexOf('RIDER') !== -1;
      if (Array.isArray(roles)) return roles.includes('RIDER');
      return false;
    },
  });
...
function runValidation(response, endpointName, isDashboard) {
  ...
  if (isDashboard) {
    checksObj[`[${endpointName}] contains dashboard keys`] = () => isJson && jsonParsed && jsonParsed.data && ('rider_id' in jsonParsed.data) && ('online' in jsonParsed.data);
  } else {
    checksObj[`[${endpointName}] contains profile keys`] = () => isJson && jsonParsed && jsonParsed.data && ('id' in jsonParsed.data) && ('vehicleType' in jsonParsed.data);
  }
...
export function runWarmup(data) {
  ...
  const dashRes = http.get(
    `${cleanBaseUrl}/api/v1/rider/dashboard`,
  ...
  const profileRes = http.get(
    `${cleanBaseUrl}/api/v1/rider/profile`,
...
export function runMeasured(data) {
  ...
  const dashRes = http.get(
    `${cleanBaseUrl}/api/v1/rider/dashboard`,
    Object.assign({}, authParams, { tags: { name: 'rider_dashboard' } })
  );
  const dashOk = runValidation(dashRes, 'rider_dashboard', true);
  riderDashboardLatency.add(dashRes.timings.duration);
  riderDashboardFailures.add(dashOk ? 0 : 1);

  // GET /api/v1/rider/profile
  const profileRes = http.get(
    `${cleanBaseUrl}/api/v1/rider/profile`,
    Object.assign({}, authParams, { tags: { name: 'rider_profile' } })
  );
  const profileOk = runValidation(profileRes, 'rider_profile', false);
  riderProfileLatency.add(profileRes.timings.duration);
  riderProfileFailures.add(profileOk ? 0 : 1);
